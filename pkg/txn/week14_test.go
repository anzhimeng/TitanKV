...
func TestConcurrentWrite(t *testing.T) {
	...
	defer func() {
        ...
        // 所以这里的清理更多是形式上的。
        txn, _ := NewTransaction(context.Background(), c)
        txn.Delete(key)
	...
	go func() {
		defer wg.Done()
		time.Sleep(50 * time.Millisecond) // 稍微错开一点
		for i := 0; i < 50; i++ {
			ctx := context.Background()
			txn, err := NewTransaction(ctx, c)
			if err != nil {
			    // 获取 TSO 失败，休眠重试
			    // t.Logf("NewTransaction failed: %v", err) // 可选
	...
	val, err := verifyTxn.Get(context.Background(), key)
	assert.NoError(t, err)
